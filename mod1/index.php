 ...
 *  Copyright notice
 *
 *  (c) 2005 Oliver Wei� (weiss@carraldo.de)
 *  All rights reserved
 *
...
$GLOBALS['LANG']->includeLLFile("EXT:wil_importcsv/mod1/locallang.xml");
$GLOBALS['BE_USER']->modAccess($MCONF, 1);    // This checks permissions and exits if the users has no permission for entry.
// DEFAULT initialization of a module [END]

class tx_wilimportcsv_module1 extends \TYPO3\CMS\Backend\Module\BaseScriptClass
{
    ...
    {
        global $BACK_PATH;
        $this->table = htmlspecialchars(GeneralUtility::_GP("table"));
        $this->pageTSConfig = BackendUtility::getPagesTSconfig($this->id);

        $this->modconfarray = BackendUtility::getModTSconfig($this->id, 'mod.web_txwilimportcsvM1');
        ...
        $this->uploadfilename = $_FILES ? $_FILES['filename']['name'] : GeneralUtility::_GP("file");
        //v($_FILES['filename']['tmp_name']);
        // move the file in the tmp folder
        $this->uploadfile = GeneralUtility::upload_to_tempfile($_FILES['filename']['tmp_name']);
        if ($this->uploadfile) {
            $source = @fopen($this->uploadfile, "r");
            while (!feof($source)) {
                $temp = fgetcsv($source, 10000, GeneralUtility::_GP('seperator'));
                $this->hiddenFileInput .= '<input type="hidden" value="' . str_replace('"', "'",
                        urlencode(serialize($temp))) . '" name="line[]" />';
            ...
            if (is_array(GeneralUtility::_GP("line"))) {
                foreach (GeneralUtility::_GP("line") as $line) {
        ...
        if (($this->id && $access) || ($GLOBALS['BE_USER']->user["admin"] && !$this->id)) {
            ...
            $this->doc->bodyTagAdditions = ' class="tx-wil_importcsv" ';
            $this->doc->styleSheetFile_post = TYPO3_MOD_PATH . 'style.css';
            ...
            $headerSection = $this->doc->getHeader(
                    "pages",
                    $this->pageinfo,
                    $this->pageinfo["_thePath"]) . "<br>" . $GLOBALS['LANG']->sL("LLL:EXT:lang/locallang_core.xml:labels.path") . ": " . GeneralUtility::fixed_lgd_cs($this->pageinfo["_thePath"],
                    50
                );
    ...
    function moduleContent()
    {
        ...
        if (is_array($this->TCA)) {
            ...
            foreach ($this->TCA["columns"] as $field => $val) {
                ...
                $this->TCA["columns"][$field]["config"] = array_merge($this->TCA["columns"][$field]["config"],
                    $overrideTCAConfigfield);
            }
        }

        if (!$this->id)  //No page in the page tree selected
        {
            $content .= '<p>' . $GLOBALS['LANG']->getLL("instruction") . '</p>';
        } else {
            ...
            switch (GeneralUtility::_GP("action")) {
                ...
                case 4: //Write into the table
                    ...
                    if (GeneralUtility::_GP("truncate_table")) {
                        if (true !== $this->truncateTable(GeneralUtility::_GP("table"))) {
                            break;
                        }
                    }
                    $viewupdates = $this->writeData(GeneralUtility::_GP("replaceArray"), 1);
                    $viewinserts = $this->writeData(GeneralUtility::_GP("insertarray"), 0);
                    if (is_array($tables)) {
                        $content .= '<h1>' . $GLOBALS['LANG']->getLL("headerForeignTables") . '</h1>';
                    ...
                    if (is_array($viewinserts)) {
                        $row = '';
                        $headerrow = '';
                        $content .= '<h1>' . $GLOBALS['LANG']->getLL("headerWritten") . ' ' . $this->table . '</h1>';
                        ...
                        unset($row);
                        $content .= '</table>';
                        $content .= '<p style="text-align: center; color: green;"><b>' . count($viewinserts) . ' ' . $GLOBALS['LANG']->getLL("successWritten") . '</b></p>';
                    }
                    //Now the Updated Rows
                    if (is_array($viewupdates)) {
                        $row = '';
                        $headerrow = '';
                        $content .= '<h1>' . $GLOBALS['LANG']->getLL("headerUpdate") . ' ' . $this->table . '</h1>';
                        $content .= '<table style="border: 1px solid grey;">';//writing the headerrow and
                        ...
                        $content .= '</table>';
                        $content .= '<p style="text-align: center; color: green;"><b>' . count($viewupdates) . ' ' . $GLOBALS['LANG']->getLL("successUpdate") . '</b></p>';
                    }
    ...
    function showUploadForm()
    {
        ...
        if (!strcmp("ALL", $this->modconfarray['tables']) || !strcmp("IGNORE", $this->modconfarray['tables'])
        ) { //Configuration allows all tables
            $result = $GLOBALS['TYPO3_DB']->sql_query('SHOW TABLES;');
        ...
        if (isset($tables) && is_array($tables)) {
            foreach ($tables as $val) {
                if ($this->isTableAllowedForThisPage($page_arr, $val)) {
                    $strTableName = $GLOBALS['LANG']->sL($GLOBALS['TCA'][$val]['ctrl']['title']);
                    $tables_ok[$val] = $strTableName . ' (' . $val . ')';
                }
            }
        }

        if (isset($tables_ok) && is_array($tables_ok)) {
            $content .= '<h3>' . $GLOBALS['LANG']->getLL('desc') . '</h3>';
            $content .= '<p>' . $GLOBALS['LANG']->getLL('instructionFile') . '</p>';
            $content .= '<table>
				<tr>
				  <td><label for="filename">' . $GLOBALS['LANG']->getLL("selectFile") . '</label></td>
				  <td><input type="file" id="filename" name="filename" size="50" /></td>
				</tr>
				<tr>
				  <td><label for="seperator">' . $GLOBALS['LANG']->getLL("selectSeperator") . '</label></td>
				  <td><input type="input" id="seperator" name="seperator" size="1" value="';
            $sep = GeneralUtility::_GP("seperator") ? GeneralUtility::_GP("seperator") : ';';
            $content .= $sep . '" /></td>
						</tr>
						<tr>
						  <td><label for="table">' . $GLOBALS['LANG']->getLL("selectTable") . '</label></td>
                          <td>';
            ...
            $content .= '</td>
			    ...
			    <tr>
			      <td><label for="checkbox_truncate_table">' . $GLOBALS['LANG']->getLL("truncateTable") . '</label></td>
            ...
            $content .= '<p>' . $GLOBALS['LANG']->getLL('noTableFound') . '</p>';
        }
    ...
    function isTableAllowedForThisPage($pid_row, $checkTable)
    {
        ...
        if ($GLOBALS['BE_USER']->check('tables_modify', $checkTable) == 0) {
            return false;
        }
        ...
        if (strstr($allowedTableList, '*') || GeneralUtility::inList($allowedTableList,
                $checkTable)
    ...
    function associateFields()
    {
        global $TCA;
        $content = '';
        $content .= '<h3>' . $GLOBALS['LANG']->getLL('headerAssociate') . '</h3>';
        $content .= '<p>' . $GLOBALS['LANG']->getLL('instructionAssociate') . '</p>';
        $type_field = $this->TCA["ctrl"]["type"];

        //Selected Type:
        $seltype = $type_field ? $this->TCA["columns"][$type_field]["config"]["default"] : key($this->TCA["types"]);
        // Standard types. But we probably overwrite it in the next step.
        ...
        if (!strcmp(1, $this->modconfarray['selectTCAtype']) && $type_field && (!strcmp(1,
                    $this->modconfarray['ignoreTCARestriction']) || !strcmp(1,
                    $GLOBALS['BE_USER']->check('non_exclude_fields', $this->table . ":" . $type_field)))
        ) { //only if set in conf.php and types are defined in TCA and (user is allowed to change the typefield or ignoreTCA is set in conf)
            $type_arr["NULL"] = "";
            ...
            if ($foreigntable) { //Type is a foreign table
                $where = '1=1';
                $where .= BackendUtility::deleteClause($foreigntable);
                $res = $GLOBALS["TYPO3_DB"]->exec_SELECTquery("uid," . $TCA[$foreigntable]["ctrl"]["label"],
                    $foreigntable, $where);
                ...
                foreach ($this->TCA["columns"][$type_field]["config"]["items"] as $key => $val) {
                    ...
                    $type_arr[$val[1]] = $GLOBALS['LANG']->sL($val[0], 1); //This is for creating the selectbox
                    $type_uids[] = $key; //and this is for making all types possible later
                }
            }
            $addparams = '&seperator=' . GeneralUtility::_GP("seperator") . '&table=' . $this->table . '&action=2&file=' . $this->uploadfilename;

            //Depending on the given values, we fill the variable $type
            if (strcmp('', GeneralUtility::_GP("type")))//type variable was sent with gpvar
            {
                $seltype = GeneralUtility::_GP("type");
            } elseif (array_key_exists("default",
                $this->TCA["columns"][$type_field]["config"]))//there is a default value set in TCA
            {
            ...
            $content .= '<h2>' . $GLOBALS['LANG']->getLL("headerType") . '</h2>';
            $content .= '<p>' . $GLOBALS['LANG']->getLL("instructionType") . '</p>';
            $content .= '<p>' . $GLOBALS['LANG']->sL(BackendUtility::getItemLabel($this->table, $type_field), 1) .
                BackendUtility::getFuncMenu($this->id, "type", $seltype, $type_arr, $script = '', $addparams) . '</p>';
        }
        ...
        $content .= '<table class="associate_table">
				  <tr>
					<th>' . $GLOBALS['LANG']->getLL("HeaderFile") . $this->uploadfilename . '</th>
					<th>&nbsp;</th>
					<th>' . $GLOBALS['LANG']->getLL("HeaderTable") . $this->table . '</th>
					<th>' . $GLOBALS['LANG']->getLL("HeaderSeperator") . '</th>
				  </tr>';
        ...
        foreach ($this->insertArray[0] as $n => $val) {
            ...
            $content .= '<tr class="' . $class . '">
						...
						<td><input type="input" name="fieldseperator[' . $n . ']" size="1" value="' . GeneralUtility::_GP("fieldseperator[" . $n . "]") . '" /></td>
					 </tr>';
        }
        $content .= '<tr height="20">
				  <td><a href="index.php?&id=' . $this->id . '&table=' . $this->table . '&seperator=' . GeneralUtility::_GP("seperator") . '">' . $GLOBALS['LANG']->getLL("backButton") . '</a></td>
				  <td>&nbsp;</td>
				  <td>
					<input type="submit" value="' . $GLOBALS['LANG']->getLL("previewButton") . '" /></p>
					<input type="hidden" name="action" value="3" />
					<input type="hidden" name="table" value="' . $this->table . '" />
					<input type="hidden" name="file" value="' . $this->uploadfilename . '" />
					<input type="hidden" name="seperator" value="' . GeneralUtility::_GP("seperator") . '" />
                    <input type="hidden" name="truncate_table" value="' . GeneralUtility::_GP("truncate_table") . '" />
				  </td>
				</tr>
			  </table>';
    ...
    function preview()
    {
        ...
        $specials = GeneralUtility::_GP('specials');
        $fieldseperator = GeneralUtility::_GP('fieldseperator');
        ...
        foreach ($this->insertArray as $temp) {
            ...
            $insertfields[$nrzeile] = GeneralUtility::_GP('typefield');//this is the typefield, which comes from GP:Vars
            //$temp =fgetcsv ($source,10000,GeneralUtility::_GP('seperator') );
            // $content.=GeneralUtility::view_array($temp);
            $value = 0;
            foreach (GeneralUtility::_GP('fields') as $nrspalte => $field) { //$nrspalte: number of column in file $field: Field name of the table,
                if ($field) {
                    ...
                    if ($this->modconfarray['charSet'] && $this->modconfarray['charSet'] != $beCharset) {
                        $insertfields[$nrzeile][$field] = iconv($this->modconfarray['charSet'], $beCharset,
                            $temp[$nrspalte]);
                            ...
                            switch ($func) {
                                case "required":
                                    if (!strcmp("", $temp[$nrspalte])) {
                                        $set = false;
                                        $error[$nrzeile][$field] = $GLOBALS['LANG']->getLL("requiredmissing") . ': ' . $temp[$nrspalte];
                                    }
                                    break;
                                case "trim":
                                ...
                                case "date":
                                    ...
                                    if (strcmp($temp[$nrspalte], "")) {
                                        if (substr_count($temp[$nrspalte], ".") >= 2) {
                                            ...
                                            $error[$nrzeile][$field] = $GLOBALS['LANG']->getLL("wrongDateFormat");
                                        }
                                    }
                                    $insertfields[$nrzeile][$field] = is_array($date) ? mktime(0, 0, 0, $date[1],
                                        $date[0], $date[2]) : "";
                                    break;
                                /* Thanks to Björn Didwischus for datetime code */
                                case "datetime":
                                    if (strcmp($temp[$nrspalte], "")) {
                                        if (preg_match("([0-9]{1,2}).([0-9]{1,2}).([0-9]{4}) ([0-9]{1,2}):([0-9]{1,2})",
                                            $temp[$nrspalte], $regs)) {
                                            $insertfields[$nrzeile][$field] = mktime($regs[4], $regs[5], 0, $regs[2],
                                                $regs[1], $regs[3]);
                                        } else {
                                            if (preg_match("([0-9]{1,2}).([0-9]{1,2}).([0-9]{4})", $temp[$nrspalte], $regs)) {
                                                $insertfields[$nrzeile][$field] = mktime(0, 0, 0, $regs[2], $regs[1],
                                                    $regs[3]);
                                            } else {
                                                $error[$nrzeile][$field] = $GLOBALS['LANG']->getLL("wrongTimeFormat");
                                ...
                                case 'lower':
                                    ...
                                    break;
                                case 'nospace':
                                    $insertfields[$nrzeile][$field] = str_replace(' ', '',
                                        $insertfields[$nrzeile][$field]);
                                    break;
                                case 'alpha':
                                    $insertfields[$nrzeile][$field] = preg_replace('[^a-zA-Z]', '',
                                        $insertfields[$nrzeile][$field]);
                                    break;
                                case 'num':
                                    $insertfields[$nrzeile][$field] = preg_replace('[^0-9]', '',
                                        $insertfields[$nrzeile][$field]);
                                    break;
                                case 'alphanum':
                                    $insertfields[$nrzeile][$field] = preg_replace('[^a-zA-Z0-9]', '',
                                        $insertfields[$nrzeile][$field]);
                                    break;
                                case 'alphanum_x':
                                    $insertfields[$nrzeile][$field] = preg_replace('[^a-zA-Z0-9_-]', '',
                                        $insertfields[$nrzeile][$field]);
                                    break;
                                default:
                                    if (substr($func, 0, 3) == 'tx_') {
                                        ...
                                        if (is_object($evalObj) && method_exists($evalObj, 'evaluateFieldValue')) {
                                            $insertfields[$nrzeile][$field] = $evalObj->evaluateFieldValue($insertfields[$nrzeile][$field],
                                                $is_in, $set);
                                        }
                                    }
                                    break;
                    ...
                    if (!strcmp("select", $configfield["type"])) {//select-field: get the int values
                        if ($configfield["foreign_table"]) { //is it a reference to another table?
                            ...
                            {
                                $temp[$nrspalte] = explode('                                          ',
                                    $temp[$nrspalte]);
                            }
                            foreach ($temp[$nrspalte] as $temp_key => $temp_val) {//Loop through the seperated (exploded) field
                                $temp_val = trim($temp_val);
                                if (strcmp("",
                                        $temp_val) && is_numeric($temp_val)
                                ) { //if it is numeric, we look if the uid exists in foreign table
                                    $where = 'uid=' . $temp_val . ' ';
                                    $where .= BackendUtility::deleteClause($configfield["foreign_table"]) . ' ';
                                    ...
                                    $res2 = $GLOBALS["TYPO3_DB"]->exec_SELECTquery("uid", $configfield["foreign_table"],
                                        $where);
                                    if (!$GLOBALS["TYPO3_DB"]->sql_num_rows($res2) > 0) {
                                        $error[$nrzeile][$field] = $GLOBALS['LANG']->getLL("noMatchingUIDFound");
                                    }
                                }
                                if (strcmp("",
                                        $temp_val) && !is_numeric($temp_val)
                                ) { //if it is not an int, we try to get the integers
            ...
            {
                $error[$nrzeile]["empty"] = $GLOBALS['LANG']->getLL("nothingToWrite");
            }
            $nrzeile++;
        }
        /* $content.=GeneralUtility::view_array($insertfields);
               $content.=GeneralUtility::view_array($nomatch);
            $content.="<hr>";*/
        // the last entry is always empty, so we can destroy it
        unset($insertfields[count($insertfields)]);
        $bInsertAll = (bool)GeneralUtility::_GP("truncate_table");
        $tablerow = '';
        foreach ($insertfields as $nrspalte => $field_array) { //Loop through insertfields, means: writing the lines. $nrspalte: number of column in file $field_array: array
            ...
            if ($error[$nrspalte]["empty"]) {
                $tablerow .= '<td colspan=99 style="color: red; background:#eee">' . $GLOBALS['LANG']->getLL("error") . ' ' . $error[$nrspalte]["empty"] . '</td>';
                ...
                if (!$bInsertAll && !empty($field_array[$uniqueField]) && $this->checkUidForReplacement($field_array[$uniqueField],
                        $this->table)
                ) //is the field "uid" filled in and replacement is set, then replace
                {
                ...
                foreach ($field_array as $field => $field_val) {//Loop: writing the fields in each line
                    if ($arrayType == 'replaceArray') {
                        $message = $GLOBALS['LANG']->getLL("updateDataRow");
                        $messageColor = 'orange';
                    } else {
                        $message = $GLOBALS['LANG']->getLL("newDataRow");
                    }
                    $background = '#fff';
                    $num = "";
                    if ($error[$nrspalte][$field]) { //Is there an error message for the field, we write it
                        ...
                        $message = $GLOBALS['LANG']->getLL("error");
                        $messageColor = 'red';
                    } else {
                        ...
                        if (is_array($nomatch[$field][$nrspalte])) { //is there a nomatch in the field, we add tempfield and add some hiddenfields
                            foreach ($nomatch[$field][$nrspalte] as $key => $nomatch_arr) {
                                if ($nomatch_unique[$field][$nrspalte . '-' . $key]) {//completely new field
                                    ...
                                    $tempfield .= '<tr><td style="background:#ffff99; color: #666">=>[row Nr. ' . strtok($num,
                                            "-") . ']</td></tr>';
        ...
        if (is_array($nomatch)) {
            $content .= '<p style="background:#ffff99">' . $GLOBALS['LANG']->getLL("foreignTableNotFound") . '</p>';
        }
        $content .= '<h1>' . $GLOBALS['LANG']->getLL("preview") . '</h1>
					<table style="border: 1px solid grey;">
					<tr><th>&nbsp;</th>';
        $typefield = GeneralUtility::_GP('typefield');
        if (is_array($typefield)) {
            $content .= '<th style="background:#FFF" >' . key($typefield) . '</th>';
        }
        foreach (GeneralUtility::_GP('fields') as $val) {
            if ($val) {
        ...
        $content .= '</table>
				...
				<input type="hidden" name="truncate_table" value="' . GeneralUtility::_GP("truncate_table") . '" />
				' . $hiddenfields . '
				<div style="float:left"> <a href="javascript:history.back()">' . $GLOBALS['LANG']->getLL("backButton") . '</a></div>
				<div style="text-align:center">
				<input type="submit" value="' . $GLOBALS['LANG']->getLL("writeButton") . '" /></div>';
    ...
    {
        if (!$_FILES) {
            $this->arErrors[] = $GLOBALS['LANG']->getLL("missingFile");
            return false;
        }
        if (!GeneralUtility::_GP('seperator')) {
            $this->arErrors[] = $GLOBALS['LANG']->getLL("missingSeperator");
            return false;
        }
        if (!$this->table) {
            $this->arErrors[] = $GLOBALS['LANG']->getLL("missingTable");
            return false;
        }
        if (!$this->checkUpload()) {
            return false;
    ...
    function checkUpload()
    {
        ...
        /* if (!GeneralUtility::_POST('syspages'))
        {
            $this->status="<br /><br /><font color='red'><strong>".$GLOBALS['LANG']->getLL("emptySyspages")."</strong></font>";
            return false;
        } */

        // Check for file extension (only CSV is allowed)
        if (!preg_match("/.csv\$/i", $_FILES['filename']['name'])) {
            $this->arErrors[] = $GLOBALS['LANG']->getLL("wrongMime");
            return false;
        }

        // Check for size
        if ($this->is_too_big($_FILES['filename']['size'])) {
            $this->arErrors[] = $GLOBALS['LANG']->getLL("fileTooBig");
            return false;
        }

        // Upload the file
        if (!$this->uploadfile) {
            $this->arErrors[] = $GLOBALS['LANG']->getLL("upError");
            return false;
        }
    ...
    function createFieldsArrayFromTCA($fields)
    {
        ...
        foreach ($temp_arr as $val) {
            if (strcmp(1, $this->TCA["columns"][$val]["exclude"]) || $GLOBALS['BE_USER']->check('non_exclude_fields',
                    $this->table . ":" . $val) || !strcmp(1, $this->modconfarray['ignoreTCARestriction'])
            ) {//field is not exclude field or user is allowed to fill the exclude field of we ignore the TCA restriction
                if ($val == "--div--") //handle the --div-- and --palette-- specials
                {
                    ...
                    {
                        $fields_arr[$val] = $GLOBALS['LANG']->sL(BackendUtility::getItemLabel($this->table, $val),
                                1) . '   (' . $val . ')';
                    }
                }
            }
        ...
        if (!strcmp(1, $this->modconfarray['allowOverwriteRows'])) {
            # was $fields_arr["uid"] = $GLOBALS['LANG']->getLL("replaceUid");
            $fields_arr[$this->modconfarray['uniqueField']] = $GLOBALS['LANG']->getLL("replaceUid");
        }
        return $fields_arr;
    }
    ...
    {
        //GeneralUtility::print_array($insertarray);
        if (is_array($insertarray)) {
            foreach ($insertarray as $insertfields) {
                $mm_table = array();
                $insertfields["val"]["pid"] = $this->id;
                if ($this->TCA["ctrl"]["tstamp"]) {
                    $insertfields["val"][$this->TCA["ctrl"]["tstamp"]] = time();
                }
                ...
                if (strcmp("", $insertfields["new"])) {
                    foreach ($insertfields["new"] as $key => $val) { //$key: name of foreigntable, $val:array
                        ...
                        foreach ($val as $val2) { // $val2:array:field, $value
                            $val2["pid"] = $this->id;
                            if ($this->TCA["ctrl"]["tstamp"]) {
                                $val2[$this->TCA["ctrl"]["tstamp"]] = time();
                            }
                            if ($this->TCA["ctrl"]["crdate"]) {
                                $val2[$this->TCA["ctrl"]["crdate"]] = time();
                            }
                            $GLOBALS['TYPO3_DB']->exec_INSERTquery($key,
                                $val2); //write the new value in the foreign table
                            ...
                            if ($GLOBALS['TYPO3_DB']->sql_insert_id()) {
                                $tables[$key][$GLOBALS['TYPO3_DB']->sql_insert_id()] = array_shift($val2);
                            } //we save all new entries in this array $tables for later (search, output)
                            unset ($insertfields["ref"][$field]);
                        }
                    }
                }
                // $content.=GeneralUtility::view_array($tables);
                ...
                foreach ($insertfields["val"] as $key => $val) {
                    ...
                    if ($this->TCA["columns"][$key]["config"]["MM"]) {
                        ...
                        $mm_table[$key] = array(
                            "mm_table" => $this->TCA["columns"][$key]["config"]["MM"],
                ...
                if (!strcmp(0, $type)) {
                    ...
                    $GLOBALS['TYPO3_DB']->exec_UPDATEquery($this->table, $uniqueField . '=' . $uniqueFieldValue,
                        $insertfields["val"]);
                }
